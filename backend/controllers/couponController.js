...
const getAvailableCoupons = async (req, res) => {
  try {
    ...
    const now = new Date();
    const user = await User.findById(userId).select("redeemedCoupons");
    const redeemedCouponIds = user.redeemedCoupons.map(coupon => coupon.coupon.toString());
    const availableCoupons = await Coupon.find({
      isActive: true,
      _id: { $nin: redeemedCouponIds }, 
      $expr: { $lt: ["$currentUses", "$totalQuantity"] }
    });
